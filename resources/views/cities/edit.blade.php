            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="parent_id" id="parent_id"
                                ...
                                @foreach($parentCities as $parentCity)
                                    <option value="{{ $parentCity->id }}" {{ old('parent_id', $city->parent_id) == $parentCity->id ? 'selected' : '' }}>
                                        {{ $parentCity->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mt-8 border-t pt-6" x-data>
                        <form id="delete-city-form" action="{{ route('cities.destroy', $city) }}" method="POST">
                            @csrf
                            @method('DELETE')
                        ...
                        <x-modal-confirm name="confirm-delete" title="Eliminar Ciudad"
                            content="¿Estás seguro de eliminar esta ciudad? Esta acción no se puede deshacer.">
                            <x-slot name="footer">
                                ...
                                <x-danger-button class="ml-3"
                                    x-on:click="document.getElementById('delete-city-form').submit()">
                                    Sí, eliminar
                                </x-danger-button>
                            </x-slot>
                        </x-modal-confirm>
